...
define draw_color (drawingarea, cr, color)
{
   variable rgba;
   
   rgba = gdk_rgba_parse (color);
   if (rgba != NULL)
     {
...
define color_swatch_new (color)
{
   variable button, area;
   
   button = gtk_button_new ();
   area = gtk_drawing_area_new ();
   () = g_signal_connect (area, "draw", &draw_color, color);
   gtk_widget_set_size_request (area, 24, 24);
   gtk_container_add (button, area);
   gtk_widget_show_all (button);
   
   return button;
}

define create_flowbox (do_widget)
{
   variable window;
   variable scrolled, flowbox, color;
   variable colors = ["AliceBlue",
		      "AntiqueWhite",
		      "AntiqueWhite1",
		      "AntiqueWhite2",
		      "AntiqueWhite3",
   ...
   gtk_window_set_title (window, "Flow Box");
   ...
   gtk_container_add (scrolled, flowbox);
   gtk_container_add (window, scrolled);
   
   foreach color (colors)
     gtk_container_add (flowbox, color_swatch_new (color));
