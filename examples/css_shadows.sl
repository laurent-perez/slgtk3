...
define create_toolbar ()
{
   variable toolbar, item, fp;

   toolbar = gtk_toolbar_new ();
   gtk_widget_set_valign (toolbar, GTK_ALIGN_CENTER);

   item = gtk_tool_button_new (NULL, NULL);
   gtk_tool_button_set_icon_name (item, "go-next");
   gtk_toolbar_insert (toolbar, item, -1);

   item = gtk_tool_button_new (NULL, NULL);
   gtk_tool_button_set_icon_name (item, "go-previous");
   gtk_toolbar_insert (toolbar, item, -1);
...
define create_css_shadows (do_widget)
{
   ...
   gtk_container_add (window, paned);
	...
	fp = fopen ("css/css_shadows.css", "r");
	lines = fgetslines (fp);
	bytes = strjoin (lines);
	() = fclose (fp);
